...
struct buddy {
    U16 mem_size;           /* memory size of allocation, not include size of management */
    U16 min_alloc_size;     
    U16 tree[2];            /* buddy tree */
};
...
{
    U16 i, try_upper_level;
    U16 level_bit_offset, level_bit_length;
    ...
    do {
        ...
        /* find in this level if there is '01' or '10', which means one of the buddy 
         * has been used, and the other is free, we just allocate the free one 
...
{       
    ...
    U16 buddy_offset, buddy_size, level_offset;
    U16 word_index, bit_index;
    struct buddy *buddy = (struct buddy *)buffer;
    ...
    level_offset = lower_pow2(buddy_offset);
    buddy_size = buddy->mem_size / level_offset;
    buffer_offset = (buddy_offset - level_offset) * buddy_size;
    while(1) {
        word_index = buddy_offset >> 4;
        bit_index  = buddy_offset & 0xf;
...
int buddy_free(S8 *buffer, S8 *tofree)
{
    ...
    /* get lowest level buddy bit offset */
    buddy_offset = (buddy->mem_size + buffer_offset) / buddy->min_alloc_size;   
    ...
    /* this situation should not happen if there isn't error in the caller. */
    ASSERT(0);
    return -1;
}
